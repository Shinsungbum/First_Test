...
.btn_wrap{text-align:center; margin:40px 0 30px; display: block;}
.btn_wrap a{cursor:pointer; color: #fff; background-color: #4380ce; font-weight: bold; padding: 10px;  height: 52px; line-height: 30px; width: 168px; display: inline-block; text-align: center; margin: 0 auto;}
...
@media ( max-width: 1023px ) {
  ...
  .btn-fill {border:1px solid #ececec;font-size:14px;color:#4c4c4c; height:37px; padding:10px; width:100%;}
}
</style>
</head>
<body>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  ...
                  <td><input type="text" name='email' value="${loginInfo.email }" readonly></td>
                </tr>
                <tr>
                  <th><span>이름</span></th>
                ...
                <tr>
                  ...
                  <td><input type="text" name='phone' value="${loginInfo.phone }" readonly></td>
                </tr>
                <tr><th><span>주소</span></th>
					<td><a class='btn-fill' onclick='post()' style="cursor:pointer;">우편번호찾기</a>
						<input type='text' name='post' style="margin-bottom: 10px;" value="${loginInfo.post }" class='w-px60' readonly>
						<input type='text' name='addr'  value="${loginInfo.addr }" class='full' readonly>
						<input type='text' name='addr_more' value="${loginInfo.addr_more }" class='full'>
					</td>
				</tr>
              </tbody>
            </table>
...
$('.delete').click(function(){
	...
	function modify() {
		...
		if ($('[name=pw]').val() == '') {
			//alert($('[name=name]').attr('title')+ '을 입력하세요');
			var title = $('[name=pw]').closest('tr').children('th').text();
			alert(title + '을 입력하세요');
			$('[name=pw]').focus();
			return;
		}
		if ($('[name=pw_ck]').val() == '') {
			//alert($('[name=name]').attr('title')+ '을 입력하세요');
			var title = $('[name=pw_ck]').closest('tr').children('th').text();
			alert(title + '을 입력하세요');
			$('[name=pw_ck]').focus();
			return;
		}
		if ($('[name=phone]').val() == '') {
			//alert($('[name=name]').attr('title')+ '을 입력하세요');
			var title = $('[name=phone]').closest('tr').children('th').text();
			alert(title + '을 입력하세요');
			$('[name=phone]').focus();
			return;
		}
