...
public class AndReviewController {
	 ...
	 * 
	 * @RequestMapping(value = "/andModifyNofile", produces =
	 * "text/html;charset=utf-8") public String modifyNOfile(String vo,
	 * HttpServletRequest request) { MemberVO modifynofile = new Gson().fromJson(vo,
	 * MemberVO.class); System.out.println("파일수정안해서 그대로인 상태");
	 * 
	 * MemberVO profile = dao.profile_image_detail(modifynofile.getEmail()); String
	 * imgpath = profile.getProfile_image(); // int result =
	 * dao.update(modifynofile); dao.update(modifynofile);
	 * 
	 * 
	 * 
	 * //이미지파일이 그대로인 경우 if(imgpath.equals( modifynofile.getProfile_image() ) ) {
	 * profile.getProfile_image(); System.out.println(profile.getProfile_image());
	 * 
	 * //기존파일 없애고 기본이미지로 했을때 이리 타야 하는데.. }else if ( !
	 * imgpath.equals(modifynofile.getProfile_image() ) ) {
	...
	public String review_insert(String vo, int id, int store_code) {
		ReviewVO reviewInfo = gson.fromJson(vo, ReviewVO.class);
