...
SELECT      first_name||' '||last_name 이름, department_id 부서코드
FROM        employees
...
SELECT      employee_id 사번, first_name||' '||last_name 이름, salary
FROM        employees
...
SELECT      employee_id 사번, first_name||' '||last_name 성명, department_id 부서코드, salary 급여    
FROM        employees
...
SELECT      employee_id 사번, first_name||' '||last_name 성명, salary "Monthly Salary"
FROM        employees
WHERE       salary BETWEEN 3000 AND 15000
AND         department_id BETWEEN 30 AND 60;


--05. 소속된 부서코드가 없는 사원의 
--사번, 성명, 업무코드를 조회하는 쿼리문을 작성한다.
SELECT      employee_id 사번, first_name||' '||last_name 성명, job_id 업무코드
FROM        employees
...
SELECT      employee_id 사번, first_name||' '||last_name 성명, salary 급여, commission_pct
FROM        employees
WHERE       commission_pct IS NOT null
