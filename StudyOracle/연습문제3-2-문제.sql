...
SELECT      first_name 이름, length(first_name) "이름의 길이"
FROM        employees
...
SELECT      first_name 이름, LPAD(salary, 15, '$' ) lpad_salary
FROM        employees
...
첫번째 '.' 의 위치      : INSTR(phone_number, '.',1,1)
마지막번째 '.'의 위치   : INSTR(phone_number,'.', -1, 1)
SELECT      employee_id 사번, last_name 성, PHONE_NUMBER 전화번호,
            SUBSTR(PHONE_NUMBER,0, INSTR(PHONE_NUMBER, '.') -1) local_number,
...
SELECT      job_id 업무코드, job_title 업무제목, SUBSTR(job_id,1 ,INSTR(job_id, '_')-1) 직무,
