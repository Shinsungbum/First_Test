...
ORDER BY    이름 ASC;            -- ORDER BY + 컬럼위치(순서번호)
--------------------------------------------------------------------------------
절           항목                                  ALIAS
--------------------------------------------------------------------------------
SELECT      필드명                                   O 
...
[예제 2-40] 80번 부서 사원의 이름, 성, 급여, 부서코드 정보를 이름으로 오름차순으로 정렬한다.
SELECT      last_name 성, first_name 이름, salary 급여, department_id 부서코드
FROM        employees
WHERE       department_id = 80
ORDER BY    이름 ASC;

[예제 2-42] 60번 부서 사원의 사번, 성, 급여, 연봉을 연봉으로 오름차순으로 정렬한다.
SELECT      employee_id 사번, last_name 성, salary 급여, salary * 12 연봉
FROM        employees
