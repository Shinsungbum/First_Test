...
ALIAS 사용방법
1. AS 키워드 사용 ☞ 컬럼의표현 AS ALIAS명
2. 공백을 두고 사용 ☞ 컬럼의표현 ALIAS명
3. ALIAS명에 공백이 있으면 쌍따옴표를 사용하고, 공백이 없으면 쌍따옴표 생략가능
...
[예제 2-14]  30번 부서에서 급여가 10000 이하이면서 2005년 이전에 입사한 사원의 
...
사번, 성명, 급여, 부서 정보를 조회한다. 
단 성명은 성과 이름을 합해서 name, 부서는 dept_id, 사번은 emp_id 로 별명한다.
...
NOT 연산자와 BETWEEN 연산자를 함께 사용하여 BETWEEN 과 반대되는 조건을 만든다.
...
사번, 성명, 급여, 부서정보를 조회한다.
SELECT  employee_id, first_name || ' ' || last_name name, salary, department_id dept_id --14
FROM    employees
WHERE   department_id = 30
OR      department_id = 60
OR      department_id = 90;
