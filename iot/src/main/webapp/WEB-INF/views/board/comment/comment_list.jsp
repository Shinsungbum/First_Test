...
<style>
.view {width:100%; }
.modify {width:100%; display: none; margin-top: 5px; padding: 0px;}
...
$div.children('.modify').val( $div.children('.view').html()
													.replace( /<br>/g,'\n' ) );
$div.children('.modify').css( 'height', $div.children('.view').height() - 5 );

display( $div, false );//수정상태
}else{
		var comment = JSON.stringify( { id: $div.data('id'), content: $div.children('.modify').val() });
	$.ajax({
		type: 'post',
